  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => {
        setSuccess(false);
        onClose();
        onProductAdded(); // Callback for parent
      }, 2000);
    } catch (err) {
      console.error('Error creating product:', err);
      setError(err instanceof Error ? err.message : 'Không thể tạo sản phẩm mới');
    }
  };
          ...
          {success && (
            <div className="p-6 bg-green-50 border-b border-green-200">
              <div className="flex items-center">
                ...
                <span className="text-green-800">Sản phẩm đã được thêm thành công!</span>
              </div>
            </div>
          )}
